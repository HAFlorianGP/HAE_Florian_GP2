...
static sf::Texture * redTex = nullptr;
...
int main()
{
	...
	unsigned int col = 0x5DE8DBff; //il est plus blanc
	whiteTex->update((const sf::Uint8*)&col,1,1,0,0);

	/*redTex = new Texture();
	if (!redTex->create(1, 1)) printf("tex crea failed\n");
	redTex->setSmooth(true);
	unsigned int col2 = 0xFF1310ff;
	redTex->update((const sf::Uint8*)&col, 1, 1, 0, 0);*/
	...
	{
		sf::Time ElapsedTimeColor = ClockColor.getElapsedTime();
		...
		float now = frameStart.asSeconds();
		float moving = frameStart.asSeconds();
		...
		window.clear(sf::Color::Black);//nettoie la frame
